...
/*
Slower than the Fenwick Tree solution.
Beecrowd = TLE
Codeforces = AC
*/
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = first_i;; j++) {
      ...
      while (diagonal.cross(p2) < 0) {
        ...
        active.insert({areas[idx_map[p2.id]].first, p2.id});
        k = (k + 1) % n;
        p2 = points[k];
      }
      ...
      auto it = active.lower_bound(make_pair(complement, -1));
      if (it != active.end()) concave_count += active.size() - active.order_of_key(*it);
    }
  }
}

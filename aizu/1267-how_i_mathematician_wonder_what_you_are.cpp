...
vector<Segment> create_half_planes(const vector<Point>& polygon) {
...
int main() {
  ...
  while (cin >> N && N > 0) {
    ...
    const vector<Segment> half_planes = create_half_planes(polygon);
    ...
    cout << fixed << setprecision(8) << (area > 0) << endl;
  }
}
